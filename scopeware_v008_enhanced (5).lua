...
-- Pre-key: if user set SCRIPT_KEY before running the loader, skip the key window.
-- We check every place a global could be: _G, getgenv (executor global env),
-- shared, and the script's own env via getfenv at multiple levels.
do
	local valid = "gzuf86GOIGugtz56678t6ff6FF7GZi678585967gg7rfhih8t"
	local backup = "SCOPEWARE-MASTER-V3-XKQZ9"
	...
	-- getgenv() — executor-level shared env
	local ok, genv = pcall(function() return getgenv and getgenv() end)
	if ok and genv then
		table.insert(candidates, tryGet(genv, "SCRIPT_KEY"))
		table.insert(candidates, tryGet(genv, "scopeware_key"))
	end
	-- shared (Roblox built-in)
	if shared then
		table.insert(candidates, tryGet(shared, "SCRIPT_KEY"))
	end
	-- Walk up function envs to find caller's globals
	for level = 0, 5 do
		local ok2, env = pcall(function() return getfenv(level) end)
		if ok2 and env then
			table.insert(candidates, tryGet(env, "SCRIPT_KEY"))
		end
	end
	for _, k in ipairs(candidates) do
		if type(k) == "string" and (k == valid or k == backup) then
			_G.sw_verified = true
			print("[scopeware] pre-key accepted, skipping key window")
			break
		end
	end
end
