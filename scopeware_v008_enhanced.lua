...
--  Calls https://scopeware-key-api.ytlilzaz.workers.dev/verify
--  HWID = Roblox UserId (unique to each Roblox account)
-- ============================================================
local KEY_API_URL = "https://scopeware-key-api.ytlilzaz.workers.dev"
local _hwid = tostring(LP.UserId)

-- verifyKey: returns (true, "ok") on success, (false, reason) on fail
local function verifyKey(key)
	...
	if not parseOk or type(data) ~= "table" then return false, "bad response" end
...
do
	...
	for _, k in ipairs(candidates) do
		if type(k) == "string" and #k > 5 then
			-- NOTE: we can't actually call verifyKey here yet because JSON isn't loaded.
			-- We just remember the candidate; real verification happens after JSON is set up.
			_G.sw_pre_key = k
			break
		end
	end
...
--  If SCRIPT_KEY was set before the loader and it's valid via the API,
--  skip the key window.
-- ============================================================
