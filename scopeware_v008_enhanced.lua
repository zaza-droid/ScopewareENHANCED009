...
-- ================================================
Sp(WL,656)
L(WL,"[ Shaders ]",0,662,200,14,H,11); Sp(WL,678)
L(WL,"Combine post-effects for a cinematic look.",0,684,400,12,C.TDIM,9)
...
L(WL,"Preset:",0,706,50,14,C.TDIM,10)
DD(WL, 56, 706, SHADER_NAMES, _shaderState, "current", function(v)
...
TB(WL, "Apply", 174, 706, 50, 18, ac(), function()
	local fn = SHADER_PRESETS[_shaderState.current]
	if fn then fn() end
end)
TB(WL, "Clear", 230, 706, 50, 18, rgb(160, 60, 80), function()
	clearShaders()
	_shaderState.current = "None"
	if notify then notify("Shaders cleared", "info", 1.5) end
end)

L(WL,(#SHADER_NAMES - 1) .. " shader presets available. [HIGH] = needs strong GPU.",0,728,400,12,C.TDIM,9)
...
local function switchTab(name)
	if curTab == name then return end  -- no-op if already on it
	local prevName = curTab
	curTab=name
	-- Animate top button highlights
	for n,b in pairs(topBtns) do
		tween(b,{BackgroundColor3=(n==name) and ac() or acDp()},TI_FAST)
	end
	-- Slide+fade transition: previous slides out left, new slides in from right
	for n,p in pairs(pageMap) do
		if n == name then
			p.Visible = true
			p.Position = UDim2.fromOffset(40, 0)
			-- Apply fade by setting transparency on all descendants briefly
			pcall(function()
				for _, ch in ipairs(p:GetDescendants()) do
					if ch:IsA("TextLabel") or ch:IsA("TextButton") or ch:IsA("Frame") then
						-- skip; just animate the page itself
					end
				end
			end)
			tween(p, {Position = UDim2.fromOffset(0, 0)}, TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out))
		elseif n == prevName then
			-- Brief slide-out before hiding (helps the eye track the transition)
			pcall(function()
				tween(p, {Position = UDim2.fromOffset(-40, 0)}, TweenInfo.new(0.16, Enum.EasingStyle.Quint, Enum.EasingDirection.In))
				task.delay(0.16, function() p.Visible = false; p.Position = UDim2.fromOffset(0, 0) end)
			end)
		else
			p.Visible = false
		end
