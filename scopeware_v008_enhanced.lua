-- ============================================================
--  scopeware  V 0.0.9  |  StarterPlayerScript
...
-- ============================================================
local TOGGLE_KEY   = "Insert"        -- Main panel toggle
...
-- ============================================================
--  UICORNER HELPER  (disabled — no rounded corners)
-- ============================================================
local function corner(inst, radius)
	-- no-op: rounded corners removed for sharp-edge look
	return nil
end
...
local function tween(inst, props, ti)
	-- Animations disabled: set properties instantly
...
-- ============================================================
--  STUDIO BYPASS
--  In Roblox Studio, HttpGet is often disabled and there are no
--  real customers — auto-verify so we can develop/test the UI.
-- ============================================================
-- Dev override: if you set _G.sw_dev_override = true BEFORE loading scopeware,
-- the key check is skipped. Use only for development/testing your own script.
if _G.sw_dev_override == true then
	_G.sw_verified = true
	_G.sw_key = "DEV-OVERRIDE"
end

-- Persistent key: read from disk if available (survives teleports + restarts)
-- This means after the first successful login, scopeware never asks for a key again.
if not _G.sw_verified then
	local okFS = type(readfile)=="function" and type(isfile)=="function"
	if okFS then
		local persistedKey = nil
		pcall(function()
			if isfile("scopeware/key.txt") then
				persistedKey = readfile("scopeware/key.txt")
				if type(persistedKey) == "string" then
					persistedKey = persistedKey:gsub("^%s+",""):gsub("%s+$","")
				end
			end
		end)
		if type(persistedKey) == "string" and #persistedKey > 5 then
			_G.sw_pre_key = persistedKey
		end
	end
end
...
if _G.sw_pre_key and not _G.sw_verified then
	...
	if ok then
		_G.sw_verified = true
		_G.sw_key = pk
		if notify then notify("Key verified ✓", "success", 2) end
		-- Persist key to disk so future executions skip the key window
		if type(writefile) == "function" and type(makefolder) == "function" then
			pcall(function()
				if not isfolder or not isfolder("scopeware") then
					pcall(function() makefolder("scopeware") end)
				end
				writefile("scopeware/key.txt", pk)
			end)
		end
	else
	end
...
local WC = {
	...
	ColorCorr=false, Saturation=0, Contrast=0, CCBrightness=0, Tint=Color3.new(1,1,1),
	CustomFog=false, FogStart=0, FogEndCustom=500, FogR=180, FogG=180, FogB=200,
	CustomExposure=false, ExposureVal=0,
	-- Weather
	Weather="None", WeatherIntensity=0.5,
	SkyboxOn=false, SkyboxId="", ArmOffset=false, ArmX=0, ArmY=0, ArmZ=0,
}
local origFog=Lighting.FogEnd; local origFogStart=Lighting.FogStart; local origFogColor=Lighting.FogColor
...
local function _tickWorld()
	if WC.FullBright then
		...
		if WC.CustomFog then
			pcall(function()
				Lighting.FogStart = WC.FogStart
				Lighting.FogEnd   = WC.FogEndCustom
				Lighting.FogColor = Color3.fromRGB(WC.FogR, WC.FogG, WC.FogB)
			end)
		end
		if WC.CustomExposure then
			pcall(function() Lighting.ExposureCompensation = WC.ExposureVal end)
		end
	end
	if not WC.CustomFog and not WC.FogRemove then
		-- Restore original fog when neither toggle is on
		pcall(function()
			Lighting.FogEnd = origFog
			Lighting.FogStart = origFogStart
			Lighting.FogColor = origFogColor
		end)
	elseif WC.FogRemove and not WC.CustomFog then
		Lighting.FogEnd = 9e8
	end
	if not WC.CustomExposure then
		pcall(function() Lighting.ExposureCompensation = origExposure end)
	end
...
local function DD(par,x,y,opts,tbl,key,cb)
	...
	bg.MouseButton1Click:Connect(function()
		...
		-- If there are many options, cap the visible height and make it scrollable
		local rowH = 18
		local maxVisible = 9   -- show at most 9 rows; rest scrolls
		local visibleCount = math.min(#opts, maxVisible)
		local panelH = visibleCount * rowH
		local needsScroll = #opts > maxVisible

		if needsScroll then
			lF=Instance.new("ScrollingFrame")
			lF.ScrollBarThickness = 4
			lF.ScrollBarImageColor3 = ac()
			lF.ScrollingDirection = Enum.ScrollingDirection.Y
			lF.CanvasSize = UDim2.fromOffset(0, #opts * rowH)
			regC(lF, "ScrollBarImageColor3", "ac")
		else
			lF=Instance.new("Frame")
		end
		lF.Size=UDim2.fromOffset(110, panelH)
		...
		-- Slide-down animation (don't clip if scrolling)
		if not needsScroll then
			lF.ClipsDescendants=true
			lF.Size=UDim2.fromOffset(110, 0)
			tween(lF,{Size=UDim2.fromOffset(110, panelH)}, TI_FAST)
		end
		for i,opt in ipairs(opts) do
			local ob=Instance.new("TextButton"); ob.Size=UDim2.fromOffset(110, rowH)
			ob.Position=UDim2.fromOffset(0,(i-1)*rowH); ob.BackgroundColor3=acDp(); ob.BorderSizePixel=0
...
-- Single clean title: "scopeware  •  v0.0.9"
local function _updateTitle()
	local hex = string.format("#%02X%02X%02X", AC.r, AC.g, AC.b)
	TL.Text = string.format(
		'<b>scopeware</b>  <font color="#5A4570">|</font>  <font color="%s" size="11">v0.0.9</font>',
		hex
	)
end
_updateTitle()
...
_G.sw_minState = _G.sw_minState or false
-- forward-declared so we can wire the Home button before switchTab exists
local goHome
...
_G.sw_closeToken = _G.sw_closeToken or 0
makeWinBtn("X", -4,  rgb(200,40,60), function()
	local myToken = _G.sw_closeToken + 1
	_G.sw_closeToken = myToken
	tween(MF,{Size=UDim2.fromOffset(MF.AbsoluteSize.X,0),BackgroundTransparency=1},TI_CLOSE)
	task.delay(0.22,function()
		if _G.sw_closeToken == myToken then
			SG.Enabled=false
...
makeWinBtn("-", -28, acDp(), function()
	_G.sw_minState=not _G.sw_minState
	tween(MF,{Size=UDim2.fromOffset(MF.Size.X.Offset, _G.sw_minState and 28 or PH)},TI_MED)
end)
-- Home button: same neutral style as minimize, no special color
local homeBtn = makeWinBtn("home", -54, acDp(), function()
	if goHome then goHome() end
end)
homeBtn.Size = UDim2.fromOffset(38, 18)  -- wider to fit "home" text
homeBtn.TextSize = 9
...
-- ================================================
--  HOME PAGE  (welcome + dashboard combined)
-- ================================================
local HOME = mkPage("Home")
do
	-- Scrollable container so it always fits
	local hSC = Instance.new("ScrollingFrame")
	hSC.Size = UDim2.new(1, 0, 1, 0); hSC.Position = UDim2.fromOffset(0, 0)
	hSC.BackgroundTransparency = 1; hSC.BorderSizePixel = 0
	hSC.ScrollBarThickness = 4; hSC.ScrollBarImageColor3 = ac()
	hSC.CanvasSize = UDim2.new(0, 0, 0, 720)
	hSC.ScrollingDirection = Enum.ScrollingDirection.Y
	hSC.ZIndex = 5; hSC.Parent = HOME
	regC(hSC, "ScrollBarImageColor3", "ac")

	-- ─── Welcome banner ─────────────────────────────
	local banner = Instance.new("Frame"); banner.Size = UDim2.fromOffset(404, 110)
	banner.Position = UDim2.fromOffset(8, 8); banner.BackgroundColor3 = acDp()
	banner.BorderSizePixel = 0; banner.ZIndex = 6; banner.Parent = hSC
	; regC(banner, "BackgroundColor3", "dp")
	-- subtle gradient
	local bGrad = Instance.new("UIGradient", banner)
	bGrad.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0, ac()),
		ColorSequenceKeypoint.new(1, acDp()),
	})
	bGrad.Rotation = 35
	bGrad.Transparency = NumberSequence.new(0.6)

	local logo = Instance.new("TextLabel")
	logo.Size = UDim2.fromOffset(380, 32); logo.Position = UDim2.fromOffset(14, 12)
	logo.BackgroundTransparency = 1; logo.RichText = true
	logo.Text = '<b>scopeware</b>  <font color="#9655FF">|</font>  <font size="14">v0.0.9</font>'
	logo.TextColor3 = rgb(240, 235, 255); logo.Font = Enum.Font.GothamBold; logo.TextSize = 22
	logo.TextXAlignment = Enum.TextXAlignment.Left; logo.ZIndex = 7; logo.Parent = banner

	local tagline = Instance.new("TextLabel")
	tagline.Size = UDim2.fromOffset(380, 18); tagline.Position = UDim2.fromOffset(14, 48)
	tagline.BackgroundTransparency = 1
	tagline.Text = "Premium Roblox enhancement · 100+ features · Lifetime access"
	tagline.TextColor3 = C.TDIM; tagline.Font = Enum.Font.Gotham; tagline.TextSize = 11
	tagline.TextXAlignment = Enum.TextXAlignment.Left; tagline.ZIndex = 7; tagline.Parent = banner

	-- discord/website links
	local linkBar = Instance.new("Frame"); linkBar.Size = UDim2.fromOffset(380, 22)
	linkBar.Position = UDim2.fromOffset(14, 76); linkBar.BackgroundTransparency = 1
	linkBar.ZIndex = 7; linkBar.Parent = banner
	local linkLayout = Instance.new("UIListLayout", linkBar)
	linkLayout.FillDirection = Enum.FillDirection.Horizontal
	linkLayout.Padding = UDim.new(0, 6); linkLayout.SortOrder = Enum.SortOrder.LayoutOrder

	local function chip(text)
		local c = Instance.new("TextLabel"); c.Size = UDim2.fromOffset(120, 20)
		c.BackgroundColor3 = rgb(20, 14, 36); c.BackgroundTransparency = 0.3
		c.BorderSizePixel = 0; c.Text = text; c.TextColor3 = ac()
		c.Font = Enum.Font.Code; c.TextSize = 9; c.ZIndex = 8; c.Parent = linkBar
		; regC(c, "TextColor3", "ac")
	end
	chip("> discord.gg/Wby7rtGXYS")
	chip("> scopeware.pages.dev")

	-- ─── Status dashboard ───────────────────────────
	L(hSC, "[ Live Status ]", 8, 132, 200, 14, H, 11)
	-- Card grid: 2x2 stat boxes
	local function statBox(x, y, label, getValue, getColor)
		local f = Instance.new("Frame"); f.Size = UDim2.fromOffset(196, 56)
		f.Position = UDim2.fromOffset(x, y); f.BackgroundColor3 = acDp()
		f.BorderSizePixel = 0; f.ZIndex = 6; f.Parent = hSC
		; regC(f, "BackgroundColor3", "dp")
		local lblObj = Instance.new("TextLabel"); lblObj.Size = UDim2.fromOffset(180, 14)
		lblObj.Position = UDim2.fromOffset(10, 6); lblObj.BackgroundTransparency = 1
		lblObj.Text = label; lblObj.TextColor3 = C.TDIM; lblObj.Font = Enum.Font.Code; lblObj.TextSize = 9
		lblObj.TextXAlignment = Enum.TextXAlignment.Left; lblObj.ZIndex = 7; lblObj.Parent = f
		local valObj = Instance.new("TextLabel"); valObj.Size = UDim2.fromOffset(180, 24)
		valObj.Position = UDim2.fromOffset(10, 22); valObj.BackgroundTransparency = 1
		valObj.Text = "—"; valObj.TextColor3 = ac(); valObj.Font = Enum.Font.GothamBold; valObj.TextSize = 17
		valObj.TextXAlignment = Enum.TextXAlignment.Left; valObj.ZIndex = 7; valObj.Parent = f
		regC(valObj, "TextColor3", "ac")
		-- live updater
		task.spawn(function()
			while f.Parent do
				local ok1, v = pcall(getValue)
				if ok1 and v then valObj.Text = tostring(v) end
				if getColor then
					local ok2, c = pcall(getColor)
					if ok2 and c then valObj.TextColor3 = c end
				end
				task.wait(0.5)
			end
		end)
	...
	statBox(8, 152, "AIMBOT",
		function() return AB.On and "ON" or "off" end,
		function() return AB.On and rgb(90, 255, 120) or C.TDIM end)
	statBox(212, 152, "ESP",
		function()
			local on = VS.Boxes or VS.Names or VS.Health or VS.Tracers or VS.Skeleton or VS.Chams
			return on and "ON" or "off"
		end,
		function()
			local on = VS.Boxes or VS.Names or VS.Health or VS.Tracers or VS.Skeleton or VS.Chams
			return on and rgb(90, 255, 120) or C.TDIM
		end)
	statBox(8, 214, "FPS",
		function() return tostring(_perfData and _perfData.fps or 0) end)
	statBox(212, 214, "PLAYERS",
		function() return tostring(#Players:GetPlayers()) end)

	-- ─── License / Account info ─────────────────────
	L(hSC, "[ License ]", 8, 280, 200, 14, H, 11)
	local licBox = Instance.new("Frame"); licBox.Size = UDim2.fromOffset(404, 50)
	licBox.Position = UDim2.fromOffset(8, 298); licBox.BackgroundColor3 = acDp()
	licBox.BorderSizePixel = 0; licBox.ZIndex = 6; licBox.Parent = hSC
	; regC(licBox, "BackgroundColor3", "dp")

	local licStatus = Instance.new("TextLabel")
	licStatus.Size = UDim2.fromOffset(380, 14); licStatus.Position = UDim2.fromOffset(12, 6)
	licStatus.BackgroundTransparency = 1
	licStatus.Text = "✓ Verified · HWID locked to Roblox UserId " .. tostring(LP.UserId)
	licStatus.TextColor3 = rgb(90, 255, 120); licStatus.Font = Enum.Font.Code; licStatus.TextSize = 10
	licStatus.TextXAlignment = Enum.TextXAlignment.Left; licStatus.ZIndex = 7; licStatus.Parent = licBox

	local keyDisplay = Instance.new("TextLabel")
	keyDisplay.Size = UDim2.fromOffset(380, 14); keyDisplay.Position = UDim2.fromOffset(12, 24)
	keyDisplay.BackgroundTransparency = 1
	-- Mask the key for security: show first 5 + last 4 only
	local maskedKey = "•••••••••"
	if _G.sw_key and #_G.sw_key > 10 then
		maskedKey = _G.sw_key:sub(1, 8) .. " · · · · · · " .. _G.sw_key:sub(-4)
	end
	keyDisplay.Text = "Key: " .. maskedKey
	keyDisplay.TextColor3 = C.TDIM; keyDisplay.Font = Enum.Font.Code; keyDisplay.TextSize = 9
	keyDisplay.TextXAlignment = Enum.TextXAlignment.Left; keyDisplay.ZIndex = 7; keyDisplay.Parent = licBox

	-- ─── What's New / Changelog ─────────────────────
	L(hSC, "[ What's New in v0.0.9 ]", 8, 360, 300, 14, H, 11)
	local CHANGELOG = {
		{tag="NEW", txt="Auto Load on Teleport — script re-runs after teleporting"},
		{tag="NEW", txt="GUI Above All toggle — panel always on top"},
		{tag="NEW", txt="File-based config system (workspace/scopeware/configs/)"},
		{tag="NEW", txt="Home page with live status dashboard"},
		{tag="FIX", txt="Key verification now uses external API"},
		{tag="FIX", txt="ESP skeleton beams now visible (thicker)"},
		{tag="UI",  txt="Cleaner title bar, reduced glow stack"},
		{tag="UI",  txt="9 crosshair styles + pulse + show-only-when-aiming"},
	}
	for i, entry in ipairs(CHANGELOG) do
		local row = Instance.new("Frame"); row.Size = UDim2.fromOffset(404, 22)
		row.Position = UDim2.fromOffset(8, 380 + (i-1)*26); row.BackgroundColor3 = acDp()
		row.BackgroundTransparency = 0.5; row.BorderSizePixel = 0; row.ZIndex = 6; row.Parent = hSC
		; regC(row, "BackgroundColor3", "dp")

		local tagLbl = Instance.new("TextLabel")
		tagLbl.Size = UDim2.fromOffset(38, 14); tagLbl.Position = UDim2.fromOffset(8, 4)
		tagLbl.BorderSizePixel = 0; tagLbl.Font = Enum.Font.GothamBold; tagLbl.TextSize = 9
		tagLbl.Text = entry.tag; tagLbl.ZIndex = 7; tagLbl.Parent = row
		
		if entry.tag == "NEW" then
			tagLbl.BackgroundColor3 = rgb(40, 180, 100); tagLbl.TextColor3 = rgb(255,255,255)
		elseif entry.tag == "FIX" then
			tagLbl.BackgroundColor3 = rgb(220, 130, 30); tagLbl.TextColor3 = rgb(255,255,255)
		else
			tagLbl.BackgroundColor3 = ac(); tagLbl.TextColor3 = rgb(255,255,255)
			regC(tagLbl, "BackgroundColor3", "ac")
		end

		local txtLbl = Instance.new("TextLabel")
		txtLbl.Size = UDim2.fromOffset(350, 22); txtLbl.Position = UDim2.fromOffset(54, 0)
		txtLbl.BackgroundTransparency = 1; txtLbl.Text = entry.txt
		txtLbl.TextColor3 = rgb(220, 210, 255); txtLbl.Font = Enum.Font.Code; txtLbl.TextSize = 10
		txtLbl.TextXAlignment = Enum.TextXAlignment.Left; txtLbl.ZIndex = 7; txtLbl.Parent = row
	end

	-- ─── Quick footer hint ───────────────────────────
	L(hSC, "Pick a tab below to start configuring scopeware.", 8, 605, 400, 14, C.TDIM, 10)
	L(hSC, "Press Insert to toggle the panel.", 8, 622, 400, 14, C.TDIM, 9)
end
...
-- Section-jump tab bar (clicking jumps to that section in the scroll)
local WTabs = Instance.new("Frame")
WTabs.Size = UDim2.new(0.48, -4, 0, 22); WTabs.Position = UDim2.fromOffset(4, 4)
WTabs.BackgroundColor3 = acDp(); WTabs.BorderSizePixel = 0; WTabs.ZIndex = 6; WTabs.Parent = WPg

regC(WTabs, "BackgroundColor3", "dp")
local WTabLayout = Instance.new("UIListLayout", WTabs)
WTabLayout.FillDirection = Enum.FillDirection.Horizontal
WTabLayout.SortOrder = Enum.SortOrder.LayoutOrder
WTabLayout.Padding = UDim.new(0, 2)

local WL=Instance.new("ScrollingFrame"); WL.Size=UDim2.new(0.48,-4,1,-30); WL.Position=UDim2.fromOffset(4,28); WL.BackgroundTransparency=1; WL.BorderSizePixel=0; WL.ScrollBarThickness=4; WL.ScrollBarImageColor3=ac(); WL.CanvasSize=UDim2.new(0,0,0,920); WL.ScrollingDirection=Enum.ScrollingDirection.Y; WL.ZIndex=5; WL.Parent=WPg; regC(WL,"ScrollBarImageColor3","ac")

-- 4 quick-jump sections
local W_SECTIONS = {
	{ name="World",    y = 0   },
	{ name="Lighting", y = 100 },  -- ambient/brightness/outdoor amb
	{ name="FX",       y = 400 },  -- color correction / fog / exposure
	{ name="Shaders",  y = 660 },  -- shader presets
}
local W_TAB_BTNS = {}

local function setActiveSubTab(name)
	for n, b in pairs(W_TAB_BTNS) do
		b.BackgroundColor3 = (n == name) and ac() or acDp()
		b.BackgroundTransparency = (n == name) and 0 or 0.4
	end
end

for i, sec in ipairs(W_SECTIONS) do
	local b = Instance.new("TextButton", WTabs)
	b.Size = UDim2.new(1/#W_SECTIONS, -2, 1, 0)
	b.BackgroundColor3 = (i == 1) and ac() or acDp()
	b.BackgroundTransparency = (i == 1) and 0 or 0.4
	b.BorderSizePixel = 0
	b.Text = sec.name; b.TextColor3 = rgb(255, 255, 255)
	b.Font = Enum.Font.GothamBold; b.TextSize = 10
	b.LayoutOrder = i; b.ZIndex = 7
	
	b.MouseButton1Click:Connect(function()
		WL.CanvasPosition = Vector2.new(0, sec.y)
		setActiveSubTab(sec.name)
	end)
	W_TAB_BTNS[sec.name] = b
end

-- Highlight the active tab as the user scrolls naturally
WL:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
	local y = WL.CanvasPosition.Y
	local current = "World"
	for _, sec in ipairs(W_SECTIONS) do
		if y >= sec.y - 30 then current = sec.name end
	end
	setActiveSubTab(current)
end)

local WR=Instance.new("ScrollingFrame"); WR.Size=UDim2.new(0.48,-4,1,-30); WR.Position=UDim2.new(0.51,0,0,28); WR.BackgroundTransparency=1; WR.BorderSizePixel=0; WR.ScrollBarThickness=4; WR.ScrollBarImageColor3=ac(); WR.CanvasSize=UDim2.new(0,0,0,400); WR.ScrollingDirection=Enum.ScrollingDirection.Y; WR.ZIndex=5; WR.Parent=WPg; regC(WR,"ScrollBarImageColor3","ac")
...
-- ================================================
--  EXTRA WORLD EFFECTS  (added in left column where space is free)
-- ================================================
Sp(WL,394)
L(WL,"[ Color Correction ]",0,400,200,14,H,11); Sp(WL,416)
Tg(WL,"Enable Color Correction",0,422,WC,"ColorCorr",function(v)
	if not v then
		local cc = Lighting:FindFirstChildOfClass("ColorCorrectionEffect")
		if cc then cc:Destroy() end
	end
end)
L(WL,"Saturation:",   0,442,80,14,C.TDIM,10); Nm(WL, 88,442,WC,"Saturation",  -1,1)
L(WL,"Contrast:",     0,460,80,14,C.TDIM,10); Nm(WL, 88,460,WC,"Contrast",    -1,1)
L(WL,"Brightness:",   0,478,80,14,C.TDIM,10); Nm(WL, 88,478,WC,"CCBrightness",-1,1)

Sp(WL,498)
L(WL,"[ Custom Fog ]",0,504,200,14,H,11); Sp(WL,520)
Tg(WL,"Custom Fog",0,526,WC,"CustomFog")
L(WL,"Start:",  0,546,40,14,C.TDIM,10); Nm(WL, 44,546,WC,"FogStart",     0,5000)
L(WL,"End:",   86,546,30,14,C.TDIM,10); Nm(WL,118,546,WC,"FogEndCustom",0,9999)
L(WL,"R:",  0,566,16,14,C.TDIM,10); Nm(WL, 18,566,WC,"FogR",0,255)
L(WL,"G:", 58,566,16,14,C.TDIM,10); Nm(WL, 76,566,WC,"FogG",0,255)
L(WL,"B:",116,566,16,14,C.TDIM,10); Nm(WL,134,566,WC,"FogB",0,255)

Sp(WL,586)
L(WL,"[ Exposure ]",0,592,200,14,H,11); Sp(WL,608)
Tg(WL,"Custom Exposure",0,614,WC,"CustomExposure")
L(WL,"Compensation:",0,634,90,14,C.TDIM,10); Nm(WL,96,634,WC,"ExposureVal",-3,3)

-- ================================================
--  WEATHER  — particle effects above the player
-- ================================================
Sp(WL,654)
L(WL,"[ Weather ]",0,660,200,14,H,11); Sp(WL,676)
L(WL,"Particle effects above the player.",0,682,400,12,C.TDIM,9)

local WEATHER_TAG = "scopeware_weather"

-- Track our weather attachment + emitters
local _weatherFollower, _weatherAtt

local function clearWeather()
	if _weatherFollower then
		pcall(function() _weatherFollower:Destroy() end)
		_weatherFollower = nil; _weatherAtt = nil
	end
	-- Also clear any orphan attachments from prior runs
	for _, p in ipairs(workspace:GetChildren()) do
		if p.Name == WEATHER_TAG then pcall(function() p:Destroy() end) end
	end
end

local function ensureWeatherFollower()
	if _weatherFollower and _weatherFollower.Parent then return _weatherFollower end
	clearWeather()
	-- Invisible part that follows the camera
	_weatherFollower = Instance.new("Part")
	_weatherFollower.Name = WEATHER_TAG
	_weatherFollower.Size = Vector3.new(0.1, 0.1, 0.1)
	_weatherFollower.Transparency = 1
	_weatherFollower.CanCollide = false
	_weatherFollower.Anchored = true
	_weatherFollower.Massless = true
	_weatherFollower.Parent = workspace
	_weatherAtt = Instance.new("Attachment", _weatherFollower)
	-- Camera-follow loop
	task.spawn(function()
		while _weatherFollower and _weatherFollower.Parent do
			local cam = workspace.CurrentCamera
			if cam then
				_weatherFollower.CFrame = CFrame.new(cam.CFrame.Position + Vector3.new(0, 30, 0))
			end
			task.wait(0.05)
		end
	end)
	return _weatherFollower
end

-- Build a particle emitter with given config
local function makeEmitter(att, config)
	local p = Instance.new("ParticleEmitter")
	p.Texture = config.texture or "rbxasset://textures/particles/smoke_main.dds"
	p.Lifetime = NumberRange.new(config.lifeMin or 1.5, config.lifeMax or 2.5)
	p.Rate = config.rate or 80
	p.Speed = NumberRange.new(config.speedMin or 30, config.speedMax or 50)
	p.SpreadAngle = Vector2.new(config.spread or 25, config.spread or 25)
	p.EmissionDirection = Enum.NormalId.Bottom
	p.Size = NumberSequence.new(config.size or 0.5)
	p.Transparency = NumberSequence.new({
		NumberSequenceKeypoint.new(0, 0.2),
		NumberSequenceKeypoint.new(1, 0.8),
	})
	p.Color = ColorSequence.new(config.color or Color3.fromRGB(200, 200, 220))
	p.Rotation = NumberRange.new(0, 360)
	p.RotSpeed = NumberRange.new(-30, 30)
	p.Acceleration = config.accel or Vector3.new(0, -50, 0)
...
local WEATHER_PRESETS = {
	["None"] = function() clearWeather() end,
	["Rain"] = function(intensity)
		clearWeather()
		local f = ensureWeatherFollower()
		makeEmitter(_weatherAtt, {
			texture = "rbxassetid://1525905166",
			lifeMin = 0.6, lifeMax = 1.0,
			rate = 600 * intensity,
			speedMin = 60, speedMax = 90, spread = 8,
			size = 0.18,
			color = Color3.fromRGB(180, 200, 220),
			accel = Vector3.new(0, -180, 0),
		})
	end,
	["Heavy Rain"] = function(intensity)
		clearWeather()
		local f = ensureWeatherFollower()
		makeEmitter(_weatherAtt, {
			texture = "rbxassetid://1525905166",
			...
			rate = 1200 * intensity,
			speedMin = 100, speedMax = 140, spread = 5,
			size = 0.22,
			color = Color3.fromRGB(160, 180, 210),
			accel = Vector3.new(0, -250, 0),
		})
	end,
	["Snow"] = function(intensity)
		...
		local f = ensureWeatherFollower()
		makeEmitter(_weatherAtt, {
			texture = "rbxassetid://272050291",
			lifeMin = 4, lifeMax = 7,
			rate = 200 * intensity,
			speedMin = 8, speedMax = 14, spread = 30,
			size = 0.5,
			color = Color3.fromRGB(255, 255, 255),
			accel = Vector3.new(0, -8, 0),
		})
	end,
	["Blizzard"] = function(intensity)
		clearWeather()
		local f = ensureWeatherFollower()
		makeEmitter(_weatherAtt, {
			texture = "rbxassetid://272050291",
			lifeMin = 2, lifeMax = 4,
			rate = 800 * intensity,
			speedMin = 40, speedMax = 70, spread = 40,
			size = 0.6,
			color = Color3.fromRGB(240, 245, 255),
			accel = Vector3.new(20, -30, 10),
		})
	end,
	["Ash"] = function(intensity)
		clearWeather()
		local f = ensureWeatherFollower()
		makeEmitter(_weatherAtt, {
			...
			rate = 100 * intensity,
			speedMin = 4, speedMax = 8, spread = 60,
			size = 0.3,
			color = Color3.fromRGB(80, 80, 80),
			accel = Vector3.new(2, -5, 1),
		})
	end,
	["Petals"] = function(intensity)
		clearWeather()
		local f = ensureWeatherFollower()
		makeEmitter(_weatherAtt, {
			texture = "rbxassetid://272050291",
			lifeMin = 6, lifeMax = 10,
			rate = 80 * intensity,
			speedMin = 3, speedMax = 8, spread = 70,
			size = 0.4,
			color = Color3.fromRGB(255, 180, 220),
			accel = Vector3.new(3, -6, 2),
		})
	end,
	["Embers"] = function(intensity)
		clearWeather()
		local f = ensureWeatherFollower()
		makeEmitter(_weatherAtt, {
			texture = "rbxasset://textures/particles/smoke_main.dds",
			lifeMin = 3, lifeMax = 6,
			rate = 150 * intensity,
			speedMin = 5, speedMax = 12, spread = 50,
			size = 0.15,
			color = Color3.fromRGB(255, 140, 60),
			accel = Vector3.new(0, 5, 0),
		})
	end,
	["Fog Drift"] = function(intensity)
		clearWeather()
		local f = ensureWeatherFollower()
		makeEmitter(_weatherAtt, {
			texture = "rbxasset://textures/particles/smoke_main.dds",
			lifeMin = 4, lifeMax = 8,
			rate = 50 * intensity,
			speedMin = 1, speedMax = 4, spread = 80,
			size = 1.5,
			color = Color3.fromRGB(220, 220, 230),
			accel = Vector3.new(2, 0, 1),
		})
	end,
	["Fireflies"] = function(intensity)
		clearWeather()
		local f = ensureWeatherFollower()
		makeEmitter(_weatherAtt, {
			texture = "rbxassetid://272050291",
			lifeMin = 3, lifeMax = 6,
			rate = 60 * intensity,
			speedMin = 1, speedMax = 3, spread = 90,
			size = 0.12,
...
local WEATHER_NAMES = {}
for n in pairs(WEATHER_PRESETS) do table.insert(WEATHER_NAMES, n) end
table.sort(WEATHER_NAMES, function(a, b)
...
local function applyWeather()
	local fn = WEATHER_PRESETS[WC.Weather]
	if fn then fn(WC.WeatherIntensity or 0.5) end
end

L(WL,"Type:",0,704,40,14,C.TDIM,10)
DD(WL, 46, 704, WEATHER_NAMES, WC, "Weather", function(v)
	WC.Weather = v
	applyWeather()
	if notify then notify("Weather: " .. v, "info", 1.5) end
end)
L(WL,"Intensity:",0,726,60,14,C.TDIM,10)
Nm(WL, 66, 726, WC, "WeatherIntensity", 0.1, 2)
TB(WL, "Refresh", 130, 726, 70, 18, ac(), function() applyWeather() end)

L(WL,(#WEATHER_NAMES - 1) .. " weather presets. Adjust intensity then click Refresh.",0,748,400,11,C.TDIM,9)

-- ================================================
--  SHADERS  — presets that combine multiple post-effects
-- ================================================
Sp(WL,768)
L(WL,"[ Shaders ]",0,774,200,14,H,11); Sp(WL,790)
L(WL,"Combine post-effects for a cinematic look.",0,796,400,12,C.TDIM,9)

-- Helper: get-or-create an effect under Lighting
...
-- Tags so we can clean up just our shaders
local SHADER_TAG = "scopeware_shader"

local function clearShaders()
	for _, c in ipairs(Lighting:GetChildren()) do
		if c.Name:sub(1, #SHADER_TAG) == SHADER_TAG then
			c:Destroy()
		end
	end
end

-- Apply a shader preset. Each preset is a function that builds effects.
local SHADER_PRESETS = {
	["None"] = function() clearShaders() end,

	["Cinematic"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.15; cc.Contrast = 0.25; cc.Brightness = 0.02
		cc.TintColor = Color3.fromRGB(255, 240, 220)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.4; bloom.Size = 24; bloom.Threshold = 0.95
	end,

	["Vintage Film"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.4; cc.Contrast = 0.15; cc.Brightness = 0.05
		cc.TintColor = Color3.fromRGB(255, 220, 180)
		local blur = getEffect("BlurEffect", SHADER_TAG.."_blur")
		blur.Size = 1
	end,
	...
	["Sepia"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -1; cc.Contrast = 0.1
		cc.TintColor = Color3.fromRGB(255, 200, 140)
	end,

	["Hot Summer"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.3; cc.Contrast = 0.15; cc.Brightness = 0.08
		cc.TintColor = Color3.fromRGB(255, 230, 200)
		local sun = getEffect("SunRaysEffect", SHADER_TAG.."_sun")
		sun.Intensity = 0.18; sun.Spread = 0.5
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.6; bloom.Size = 18; bloom.Threshold = 1.0
	end,

	["Cold Winter"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
	...
	["Night Vision"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -1; cc.Contrast = 0.5; cc.Brightness = 0.3
		cc.TintColor = Color3.fromRGB(120, 255, 120)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.8; bloom.Size = 32; bloom.Threshold = 0.7
	end,

	["Thermal"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 1; cc.Contrast = 0.8; cc.Brightness = 0.1
	...
	["Neon"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 1; cc.Contrast = 0.3; cc.Brightness = 0.05
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 1.2; bloom.Size = 36; bloom.Threshold = 0.8
	end,

	["Anime"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.5; cc.Contrast = 0.2; cc.Brightness = 0.08
		cc.TintColor = Color3.fromRGB(255, 245, 250)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.5; bloom.Size = 22; bloom.Threshold = 0.9
	end,

	["Horror"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.7; cc.Contrast = 0.4; cc.Brightness = -0.15
		cc.TintColor = Color3.fromRGB(180, 200, 220)
		local blur = getEffect("BlurEffect", SHADER_TAG.."_blur")
		blur.Size = 2
	end,
	...
	["Dreamy"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.2; cc.Contrast = -0.1; cc.Brightness = 0.1
		cc.TintColor = Color3.fromRGB(255, 230, 250)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.7; bloom.Size = 30; bloom.Threshold = 0.85
		local blur = getEffect("BlurEffect", SHADER_TAG.."_blur")
		blur.Size = 1
	end,
	...
	["Cyberpunk"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.6; cc.Contrast = 0.4; cc.Brightness = -0.05
		cc.TintColor = Color3.fromRGB(220, 180, 255)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 1.5; bloom.Size = 40; bloom.Threshold = 0.75
	end,

	["Sunset"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.4; cc.Contrast = 0.2; cc.Brightness = 0
		cc.TintColor = Color3.fromRGB(255, 180, 140)
		local sun = getEffect("SunRaysEffect", SHADER_TAG.."_sun")
		sun.Intensity = 0.3; sun.Spread = 0.7
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.8; bloom.Size = 26; bloom.Threshold = 0.9
	end,

	["Underwater"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.3; cc.Contrast = 0; cc.Brightness = -0.1
		cc.TintColor = Color3.fromRGB(140, 200, 230)
		local blur = getEffect("BlurEffect", SHADER_TAG.."_blur")
		blur.Size = 4
	end,
	...
	["High Contrast"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.4; cc.Contrast = 0.8; cc.Brightness = 0
	end,
	...
	-- ─── HIGH-GPU shaders (combine 4-5 effects, taxing on weaker devices) ───

	["Glass [HIGH]"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.1; cc.Contrast = 0.15; cc.Brightness = 0.1
		cc.TintColor = Color3.fromRGB(220, 240, 255)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 1.4; bloom.Size = 50; bloom.Threshold = 0.7
		local blur = getEffect("BlurEffect", SHADER_TAG.."_blur")
		blur.Size = 3
		local dof = getEffect("DepthOfFieldEffect", SHADER_TAG.."_dof")
		dof.FarIntensity = 0.4; dof.FocusDistance = 8; dof.InFocusRadius = 5
		dof.NearIntensity = 0.3
	end,

	["Cinematic Pro [HIGH]"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.1; cc.Contrast = 0.4; cc.Brightness = 0.05
		cc.TintColor = Color3.fromRGB(255, 235, 215)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.7; bloom.Size = 32; bloom.Threshold = 0.85
		local sun = getEffect("SunRaysEffect", SHADER_TAG.."_sun")
		sun.Intensity = 0.15; sun.Spread = 0.6
		local dof = getEffect("DepthOfFieldEffect", SHADER_TAG.."_dof")
		dof.FarIntensity = 0.5; dof.FocusDistance = 30; dof.InFocusRadius = 15
		dof.NearIntensity = 0.2
	end,

	["Hollywood [HIGH]"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.05; cc.Contrast = 0.5; cc.Brightness = 0
		cc.TintColor = Color3.fromRGB(255, 220, 200)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 1.0; bloom.Size = 40; bloom.Threshold = 0.8
		local sun = getEffect("SunRaysEffect", SHADER_TAG.."_sun")
		sun.Intensity = 0.25; sun.Spread = 0.7
		local blur = getEffect("BlurEffect", SHADER_TAG.."_blur")
		blur.Size = 1
		local dof = getEffect("DepthOfFieldEffect", SHADER_TAG.."_dof")
		dof.FarIntensity = 0.3; dof.FocusDistance = 20; dof.InFocusRadius = 10
	end,

	["Dreamscape [HIGH]"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.3; cc.Contrast = -0.05; cc.Brightness = 0.15
		cc.TintColor = Color3.fromRGB(255, 220, 240)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 1.8; bloom.Size = 56; bloom.Threshold = 0.7
		local blur = getEffect("BlurEffect", SHADER_TAG.."_blur")
		blur.Size = 2
		local dof = getEffect("DepthOfFieldEffect", SHADER_TAG.."_dof")
		dof.FarIntensity = 0.6; dof.FocusDistance = 15; dof.InFocusRadius = 8
	end,

	["Apocalypse [HIGH]"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.5; cc.Contrast = 0.6; cc.Brightness = -0.1
		cc.TintColor = Color3.fromRGB(255, 180, 120)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.9; bloom.Size = 28; bloom.Threshold = 0.85
		local sun = getEffect("SunRaysEffect", SHADER_TAG.."_sun")
		sun.Intensity = 0.4; sun.Spread = 0.9
		local blur = getEffect("BlurEffect", SHADER_TAG.."_blur")
		blur.Size = 1
	end,

	["Ocean Deep [HIGH]"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.4; cc.Contrast = 0.05; cc.Brightness = -0.2
		cc.TintColor = Color3.fromRGB(80, 180, 220)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.5; bloom.Size = 30; bloom.Threshold = 0.9
		local blur = getEffect("BlurEffect", SHADER_TAG.."_blur")
		blur.Size = 6
		local dof = getEffect("DepthOfFieldEffect", SHADER_TAG.."_dof")
		dof.FarIntensity = 0.7; dof.FocusDistance = 12; dof.InFocusRadius = 6
	end,

	["Retro VHS [HIGH]"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.5; cc.Contrast = 0.25; cc.Brightness = 0.05
		cc.TintColor = Color3.fromRGB(255, 200, 220)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.8; bloom.Size = 22; bloom.Threshold = 0.85
		local blur = getEffect("BlurEffect", SHADER_TAG.."_blur")
		blur.Size = 2
	end,
	...
	["Pastel"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.25; cc.Contrast = -0.05; cc.Brightness = 0.12
		cc.TintColor = Color3.fromRGB(255, 240, 250)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.4; bloom.Size = 24; bloom.Threshold = 0.9
	end,

	["Vibrant"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.7; cc.Contrast = 0.3; cc.Brightness = 0.05
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.6; bloom.Size = 26; bloom.Threshold = 0.8
	end,

	["Toxic"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.4; cc.Contrast = 0.35; cc.Brightness = 0
		cc.TintColor = Color3.fromRGB(160, 255, 100)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.7; bloom.Size = 30; bloom.Threshold = 0.85
	end,
	...
	["Crimson"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.2; cc.Contrast = 0.4; cc.Brightness = -0.05
	...
	["Ice Blue"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.2; cc.Contrast = 0.3; cc.Brightness = 0.1
	...
	["Faded"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.5; cc.Contrast = -0.1; cc.Brightness = 0.05
	end,
	...
	["Inferno"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.8; cc.Contrast = 0.5; cc.Brightness = 0
		cc.TintColor = Color3.fromRGB(255, 140, 60)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 1.2; bloom.Size = 35; bloom.Threshold = 0.75
	end,

	["Acid Trip [HIGH]"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 1.5; cc.Contrast = 0.4; cc.Brightness = 0.1
		cc.TintColor = Color3.fromRGB(200, 255, 220)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 2.0; bloom.Size = 60; bloom.Threshold = 0.6
		local blur = getEffect("BlurEffect", SHADER_TAG.."_blur")
		blur.Size = 4
	end,
	...
	["Forest"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.3; cc.Contrast = 0.2; cc.Brightness = 0
		cc.TintColor = Color3.fromRGB(180, 255, 180)
	end,

	["Desert"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.2; cc.Contrast = 0.3; cc.Brightness = 0.08
		cc.TintColor = Color3.fromRGB(255, 220, 160)
		local sun = getEffect("SunRaysEffect", SHADER_TAG.."_sun")
		sun.Intensity = 0.2; sun.Spread = 0.5
	end,

	["Mint"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.1; cc.Contrast = 0.05; cc.Brightness = 0.1
		cc.TintColor = Color3.fromRGB(180, 255, 220)
	end,

	["Lavender"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.05; cc.Contrast = 0; cc.Brightness = 0.08
	...
	["Coral"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.2; cc.Contrast = 0.15; cc.Brightness = 0.1
	...
	["Yellow Filter"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.3; cc.Contrast = 0.25; cc.Brightness = 0.1
		cc.TintColor = Color3.fromRGB(255, 240, 140)
	end,

	["Gameboy"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.8; cc.Contrast = 0.4; cc.Brightness = 0
		cc.TintColor = Color3.fromRGB(180, 220, 100)
	end,

	["Pixel Pop"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 1.2; cc.Contrast = 0.5; cc.Brightness = 0.05
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.4; bloom.Size = 16; bloom.Threshold = 0.95
	end,

	["Soft Glow"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0; cc.Contrast = 0; cc.Brightness = 0.05
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.6; bloom.Size = 28; bloom.Threshold = 0.9
	end,

	["Gritty"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.3; cc.Contrast = 0.6; cc.Brightness = -0.1
	end,

	["Punchy"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.5; cc.Contrast = 0.6; cc.Brightness = 0
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.4; bloom.Size = 18; bloom.Threshold = 0.95
	end,

	["Velvet"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.2; cc.Contrast = 0.2; cc.Brightness = -0.05
		cc.TintColor = Color3.fromRGB(255, 190, 220)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.7; bloom.Size = 30; bloom.Threshold = 0.85
	end,
	...
	["Polaroid"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.2; cc.Contrast = 0.1; cc.Brightness = 0.15
		cc.TintColor = Color3.fromRGB(255, 245, 220)
	end,

	["Frostbite [HIGH]"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = -0.4; cc.Contrast = 0.3; cc.Brightness = 0.05
		cc.TintColor = Color3.fromRGB(180, 220, 255)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 1.0; bloom.Size = 42; bloom.Threshold = 0.8
		local blur = getEffect("BlurEffect", SHADER_TAG.."_blur")
		blur.Size = 2
		local dof = getEffect("DepthOfFieldEffect", SHADER_TAG.."_dof")
		dof.FarIntensity = 0.4; dof.FocusDistance = 25; dof.InFocusRadius = 12
	end,

	["Galaxy [HIGH]"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.6; cc.Contrast = 0.3; cc.Brightness = -0.1
		cc.TintColor = Color3.fromRGB(180, 160, 255)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 1.6; bloom.Size = 48; bloom.Threshold = 0.7
		local blur = getEffect("BlurEffect", SHADER_TAG.."_blur")
		blur.Size = 1
	end,

	["Mystic Forest [HIGH]"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.4; cc.Contrast = 0.2; cc.Brightness = -0.05
		cc.TintColor = Color3.fromRGB(160, 230, 180)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 0.9; bloom.Size = 36; bloom.Threshold = 0.85
		local sun = getEffect("SunRaysEffect", SHADER_TAG.."_sun")
	...
	["Cathedral [HIGH]"] = function()
		clearShaders()
		local cc = getEffect("ColorCorrectionEffect", SHADER_TAG.."_cc")
		cc.Saturation = 0.1; cc.Contrast = 0.4; cc.Brightness = 0
		cc.TintColor = Color3.fromRGB(255, 220, 180)
		local bloom = getEffect("BloomEffect", SHADER_TAG.."_bloom")
		bloom.Intensity = 1.4; bloom.Size = 50; bloom.Threshold = 0.75
		local sun = getEffect("SunRaysEffect", SHADER_TAG.."_sun")
		sun.Intensity = 0.5; sun.Spread = 1.0
		local dof = getEffect("DepthOfFieldEffect", SHADER_TAG.."_dof")
		dof.FarIntensity = 0.3; dof.FocusDistance = 40; dof.InFocusRadius = 20
	end,
}

-- Build the dropdown of preset names (sorted, with None first)
local SHADER_NAMES = { "None" }
for n in pairs(SHADER_PRESETS) do
	if n ~= "None" then table.insert(SHADER_NAMES, n) end
end
table.sort(SHADER_NAMES, function(a, b)
	if a == "None" then return true end
	if b == "None" then return false end
	return a < b
end)

local _shaderState = { current = "None" }
L(WL,"Preset:",0,818,50,14,C.TDIM,10)
DD(WL, 56, 818, SHADER_NAMES, _shaderState, "current", function(v)
	local fn = SHADER_PRESETS[v]
	if fn then
		fn()
		if notify then notify("Shader: " .. v, "info", 1.5) end
	end
end)

-- Manual control buttons
TB(WL, "Apply", 174, 818, 50, 18, ac(), function()
	local fn = SHADER_PRESETS[_shaderState.current]
	if fn then fn() end
end)
TB(WL, "Clear", 230, 818, 50, 18, rgb(160, 60, 80), function()
	clearShaders()
	_shaderState.current = "None"
	if notify then notify("Shaders cleared", "info", 1.5) end
end)

L(WL,(#SHADER_NAMES - 1) .. " shader presets available. [HIGH] = needs strong GPU.",0,840,400,12,C.TDIM,9)
...
SP.CanvasSize = UDim2.new(0,0,0,1220)  -- Fits all sections including font picker
...
regC(prevBox,"BackgroundColor3","ac")
local function onAC()
...
local function serCfg()
	return JSON.encode({
		...
		AB_Predict=AB.Predict, AB_PredictStrength=AB.PredictStrength,
		AB_HitboxExp=AB.HitboxExp, AB_MinHP=AB.MinHP,
		AB_Silent=AB.Silent, AB_NearestPart=AB.NearestPart,
		AB_Priority=AB.Priority, AB_AimKey=AB.AimKey,
		AB_ClickRate=AB.ClickRate,
		-- Ragebot
		...
		VS_Skeleton=VS.Skeleton, VS_TeamColor=VS.TeamColor, VS_ShowTeam=VS.ShowTeam,
		...
		WC_CustomOutAmb=WC.CustomOutAmb, WC_OAR=WC.OAR, WC_OAG=WC.OAG, WC_OAB=WC.OAB,
		WC_CustomShadow=WC.CustomShadow, WC_ShadowVal=WC.ShadowVal,
		WC_ColorCorr=WC.ColorCorr, WC_Saturation=WC.Saturation,
		WC_Contrast=WC.Contrast, WC_CCBrightness=WC.CCBrightness,
		WC_CustomFog=WC.CustomFog, WC_FogStart=WC.FogStart, WC_FogEndCustom=WC.FogEndCustom,
		WC_FogR=WC.FogR, WC_FogG=WC.FogG, WC_FogB=WC.FogB,
		WC_CustomExposure=WC.CustomExposure, WC_ExposureVal=WC.ExposureVal,
		WC_SkyboxOn=WC.SkyboxOn, WC_SkyboxId=WC.SkyboxId,
		WC_Weather=WC.Weather, WC_WeatherIntensity=WC.WeatherIntensity,
		-- Shader
		SH_Current=(_shaderState and _shaderState.current) or "None",
		-- Accent
		...
		-- Font
		FONT=(_fontState and _fontState.font) or "Code",
		-- Script options
		SO_AutoLoad=SCRIPT_OPTS.AutoLoad, SO_TopMost=SCRIPT_OPTS.TopMost,
	})
end
...
local function desrCfg(jsonStr)
	...
	set(WC,"CustomOutAmb","WC_CustomOutAmb"); set(WC,"OAR","WC_OAR")
	set(WC,"OAG","WC_OAG"); set(WC,"OAB","WC_OAB")
	set(WC,"CustomShadow","WC_CustomShadow"); set(WC,"ShadowVal","WC_ShadowVal")
	set(WC,"ColorCorr","WC_ColorCorr")
	set(WC,"Saturation","WC_Saturation")
	set(WC,"Contrast","WC_Contrast")
	set(WC,"CCBrightness","WC_CCBrightness")
	set(WC,"CustomFog","WC_CustomFog")
	set(WC,"FogStart","WC_FogStart"); set(WC,"FogEndCustom","WC_FogEndCustom")
	set(WC,"FogR","WC_FogR"); set(WC,"FogG","WC_FogG"); set(WC,"FogB","WC_FogB")
	set(WC,"CustomExposure","WC_CustomExposure"); set(WC,"ExposureVal","WC_ExposureVal")
	set(WC,"SkyboxOn","WC_SkyboxOn"); set(WC,"SkyboxId","WC_SkyboxId")
	set(WC,"Weather","WC_Weather"); set(WC,"WeatherIntensity","WC_WeatherIntensity")
	-- Aimbot extras
	set(AB,"Predict","AB_Predict"); set(AB,"PredictStrength","AB_PredictStrength")
	set(AB,"HitboxExp","AB_HitboxExp"); set(AB,"MinHP","AB_MinHP")
	set(AB,"Silent","AB_Silent"); set(AB,"NearestPart","AB_NearestPart")
	set(AB,"Priority","AB_Priority"); set(AB,"AimKey","AB_AimKey")
	set(AB,"ClickRate","AB_ClickRate")
	-- Visuals extras
	set(VS,"Skeleton","VS_Skeleton"); set(VS,"TeamColor","VS_TeamColor"); set(VS,"ShowTeam","VS_ShowTeam")
	-- Accent
	if t.AC_r then
	...
	-- Font
	if t.FONT and _fontState then
		_fontState.font = t.FONT
		if applyFont then pcall(applyFont, t.FONT) end
	end
	-- Script options
	if t.SO_AutoLoad ~= nil then SCRIPT_OPTS.AutoLoad = t.SO_AutoLoad end
	if t.SO_TopMost ~= nil then
		SCRIPT_OPTS.TopMost = t.SO_TopMost
		-- Re-apply topmost
		pcall(function()
	...
	-- Shader
	if t.SH_Current and SHADER_PRESETS then
		local fn = SHADER_PRESETS[t.SH_Current]
		if fn then
			pcall(fn)
			if _shaderState then _shaderState.current = t.SH_Current end
		end
	end
	-- Weather
	if applyWeather then pcall(applyWeather) end
...
local fcStatus = L(SP,"",8,786,400,18,C.TDIM,10)
...
local function refreshFileList()
	...
	if not fileSysAvailable() then
		fcStatus.Text = "X Studio / your executor doesn't support file configs"
		fcStatus.TextColor3 = C.DIS
		-- Hide unusable controls so they don't overlap other settings
		fcNameBox.Visible = false
		fcList.Visible = false
		return
	end
	fcNameBox.Visible = true
	fcList.Visible = true
	local files = {}
	...
	for i, fpath in ipairs(files) do
		...
		loadBtn.MouseButton1Click:Connect(function()
			...
			if okLoad then
				...
				if notify then notify("Config loaded: " .. name, "success", 2) end
			else
				fcStatus.Text = "X Load error: " .. tostring(msg)
				fcStatus.TextColor3 = C.DIS
				if notify then notify("Load failed: " .. tostring(msg), "error", 3) end
			end
		end)
...
TB(SP,"Save to file",194,590,100,18,acMd(),function()
	...
	if ok then
		...
		fcNameBox.Text = ""
		refreshFileList()
		if notify then notify("Config saved: " .. name, "success", 2) end
	else
		fcStatus.Text = "X Save failed: " .. tostring(err)
		fcStatus.TextColor3 = C.DIS
		if notify then notify("Save failed", "error", 3) end
	end
end)
...
-- ============================================================
--  FONT PICKER — apply a chosen font to every text element
-- ============================================================
L(SP,"[ Font Customization ]",8,1086,300,14,H,11); Sp(SP,1102)
L(SP,"Pick a font for all panel text. Affects everything.",8,1108,400,12,C.TDIM,9)

-- Roblox built-in fonts (Enum.Font members that look distinct)
local FONT_OPTIONS = {
	"Code",          -- monospace, current default
	"Gotham",        -- clean modern
	"GothamMedium",
	"GothamBold",
	"GothamSemibold",
	"GothamBlack",
	"SourceSans",    -- classic Roblox
	"SourceSansBold",
	"SourceSansItalic",
	"SourceSansLight",
	"SourceSansSemibold",
	...
	"Highway",       -- condensed
	"SciFi",         -- futuristic
	"Bodoni",        -- elegant serif
	"Antique",       -- old-school
	"Arcade",        -- retro pixel
	"Fantasy",       -- decorative
	"Legacy",        -- old Roblox font
	"Roboto",
	"RobotoCondensed",
	"RobotoMono",
	"PermanentMarker",
...
-- Resolve a font name to an Enum.Font value (some names may not exist)
...
-- Apply font recursively to every text-bearing instance under a root
local function applyFontTree(root, fontEnum)
	if not root then return end
	for _, child in ipairs(root:GetDescendants()) do
		if child:IsA("TextLabel") or child:IsA("TextButton") or child:IsA("TextBox") then
			pcall(function() child.Font = fontEnum end)
		end
	end
end

local function applyFont(name)
	local fontEnum = resolveFont(name)
	-- Apply to all our ScreenGuis
	applyFontTree(SG, fontEnum)
	if EXP_SG then applyFontTree(EXP_SG, fontEnum) end
	if XHSG then applyFontTree(XHSG, fontEnum) end
	if KG then applyFontTree(KG, fontEnum) end
end

local _fontState = { font = "Code" }
L(SP,"Font:",8,1128,40,14,C.TDIM,10)
DD(SP, 52, 1128, FONT_OPTIONS, _fontState, "font", function(v)
	applyFont(v)
end)

-- Preview label (uses currently-selected font, updates live)
local fontPreview = L(SP, "The quick brown fox jumps over the lazy dog. 0123456789", 8, 1150, 400, 18, ac(), 12)
fontPreview.TextWrapped = true
regC(fontPreview, "TextColor3", "ac")
-- Hook DD callback to also update the preview
do
	local origCb
	-- Find the currently-set callback isn't trivial, so we'll re-bind:
	-- The DD function's callback is fired in addition to setting _fontState.font.
	-- We just spawn a watcher to update the preview when the state changes.
	task.spawn(function()
		local last = _fontState.font
		while fontPreview.Parent do
			if _fontState.font ~= last then
				last = _fontState.font
				pcall(function() fontPreview.Font = resolveFont(last) end)
			end
			task.wait(0.2)
		end
	end)
...
local function switchTab(name)
	...
	curTab = name
	-- Update button highlights instantly
	for n, b in pairs(topBtns) do
		b.BackgroundColor3 = (n == name) and ac() or acDp()
	end
	-- Show selected page, hide all others. No animation, no delay.
	for n, p in pairs(pageMap) do
		p.Visible = (n == name)
		p.Position = UDim2.fromOffset(0, 0)
	end
end
-- Wire the home button (forward-declared earlier)
goHome = function() switchTab("Home") end
for n,b in pairs(topBtns) do b.MouseButton1Click:Connect(function() switchTab(n) end) end
switchTab("Home")  -- start on home page
...
UserInputService.InputBegan:Connect(function(inp,gpe)
	...
	local kc = Enum.KeyCode[TOGGLE_KEY] or Enum.KeyCode.Insert
	if inp.KeyCode == kc then
		if SG.Enabled then
			...
			SG.Enabled=true
			MF.BackgroundTransparency = 0.05
			local targetH=_G.sw_minState and 28 or PH
...
-- ================================================
-- ================================================
--  SCOPEWARE EXPLORER  v2 — Horizontal Icon Bar
--  8 icons, click to open a popup panel.
-- ================================================
	...
	local function tryKey()
		...
		task.spawn(function()
			...
			if ok then
				...
				_G.sw_verified=true
				_G.sw_key=raw
				-- Persist key to disk so future executions skip this window
				if type(writefile) == "function" then
					pcall(function()
						if not isfolder or not isfolder("scopeware") then
							pcall(function() makefolder("scopeware") end)
						end
						writefile("scopeware/key.txt", raw)
					end)
				end
				tween(KW,
...
local function setupAutoLoad()
	...
	local function tryQueue()
		...
		if ok then
			if notify then notify("Reloading on next server...", "info", 1.5) end
		else
	...
	if _G.sw_autoload_resumed then
		...
		if notify then notify("scopeware reloaded ✓", "success", 2) end
	end
end
pcall(setupAutoLoad)
